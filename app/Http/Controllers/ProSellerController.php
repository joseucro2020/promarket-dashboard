...
class ProSellerController extends Controller
{
    public function index()
    {
        ...
        ->where('pro_seller', User::IS_PRO)
        ->latest()
        ->get();

        $states = Estado::where('pais_id', Pais::VENEZUELA_ID)->get();

        return view('panel.pro-sellers.index', ['clients' => $clients, 'states' => $states]);
    }
    ...
    {
        $client = User::find($id);
        $client->pro_seller = User::IS_NOT_PRO;
        $client->save();
    ...
    {
        $clients = User::select('id', 'name', 'identificacion', 'persona', 'created_at', 'telefono', 'status')
            ->where('nivel', '1')->where('status', 1)->where('pro_seller', User::IS_PRO)->get();
        return $clients;
    }
    ...
    public function getBalance($sellerId)
    {
        ...
        foreach($balance as $item) {
            if ($item->parent_class == CouponUser::CLASS_VALUE) {
                $item->load('parent.purchase.exchange');
            }
        }

        return view('panel.pro-sellers.reports', ['balance' => $balance]);
    }
