...
- **Listados y detalles**: use siempre el componente de `datatable` del panel para mostrar tablas (ya lo usan `resources/views/panel/exchange_rates/index.blade.php` o `resources/views/panel/products/index.blade.php`), tanto para la vista de lista como para los detalles/ediciones. Evite crear tablas sin `datatable`, y reutilice la configuración existente (`js/scripts/datatables/`).
 - **Listados y detalles**: use siempre el componente de `datatable` del panel para mostrar tablas (ya lo usan `resources/views/panel/exchange_rates/index.blade.php` o `resources/views/panel/products/index.blade.php`), tanto para la vista de lista como para los detalles/ediciones. Evite crear tablas sin `datatable`, y reutilice la configuración existente (`js/scripts/datatables/`).
	 - **Server-side processing**: para tablas con gran volumen de datos o cuando la paginación/búsqueda/ordenamiento debe manejarse en el servidor, implemente DataTables en modo server-side (`serverSide: true`, `processing: true`) y entregue desde el controlador JSON con `draw`, `recordsTotal`, `recordsFiltered` y `data`. Asegúrese de implementar búsqueda global, ordenamiento y paginación en la consulta del servidor.
...
- **Iconos (acciones estándar):** Para tablas de módulos, use el mismo patrón de acciones que en `special-categories`: switch de estado (`custom-switch`), botón editar `btn-flat-success` con `data-feather="edit"`, y botón eliminar `btn-flat-danger` con `data-feather="trash"`. Mantenga ese orden y alineación.
