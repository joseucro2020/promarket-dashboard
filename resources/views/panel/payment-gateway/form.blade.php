  ...
  <div class="row">
    <div class="col-12">
      <div class="card">
          ...
          <h4 class="card-title">{{ $gateway ? __('locale.Edit Payment Gateway') : __('locale.New Payment Gateway') }}</h4>
        </div>
        <div class="card-body">
          @if ($errors->any())
          ...
          <form method="POST" action="{{ $gateway ? route('payment-gateway.update', $gateway->id) : route('payment-gateway.store') }}" enctype="multipart/form-data">
            ...
            <div class="row">
              ...
              <div class="col-md-6">
                <div class="form-group">
                  <label>{{ __('locale.Provider') }}</label>
              ...
              <div class="col-md-4">
                <div class="form-group">
                  <label>{{ __('locale.Currency') }}</label>
                  <input type="text" class="form-control" name="currency" value="{{ old('currency', $gateway->currency ?? '') }}" placeholder="{{ __('locale.USD / VES') }}">
              ...
              <div class="col-md-4">
                <div class="form-group">
                  ...
                  <select class="form-control" name="status" required>
                    <option value="1" {{ old('status', isset($gateway) ? (int)$gateway->status : 1) == 1 ? 'selected' : '' }}>{{ __('locale.Active') }}</option>
                    <option value="0" {{ old('status', isset($gateway) ? (int)$gateway->status : 1) == 0 ? 'selected' : '' }}>{{ __('locale.Inactive') }}</option>
                  </select>
              ...
              <div class="col-md-6">
                <div class="form-group">
                  <label>{{ __('locale.Payment Method Code') }}</label>
              ...
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <select class="form-control" name="type">
                    <option value="" {{ old('type', $gateway->type ?? '') === '' ? 'selected' : '' }}>{{ __('locale.Auto') }}</option>
                    <option value="unique" {{ old('type', $gateway->type ?? '') === 'unique' ? 'selected' : '' }}>{{ __('locale.Unique') }}</option>
                    <option value="multi" {{ old('type', $gateway->type ?? '') === 'multi' ? 'selected' : '' }}>{{ __('locale.Multi') }}</option>
                  </select>
              ...
              <div class="col-md-12">
                <div class="form-group">
                  <label class="d-block">{{ __('locale.Available Types') }}</label>
                  @php
                  ...
                  <div class="d-flex flex-wrap" style="gap: 1rem;">
                    <div class="custom-control custom-checkbox">
                      ...
                      <label class="custom-control-label" for="avail_unique">{{ __('locale.Unique') }}</label>
                    </div>
                    <div class="custom-control custom-checkbox">
                      ...
                      <label class="custom-control-label" for="avail_multi">{{ __('locale.Multi') }}</label>
                    </div>
                  </div>
              ...
              <div class="col-md-12">
                <div class="form-group">
                  <label>{{ __('locale.Config (JSON)') }}</label>
              ...
              <div class="col-md-12">
                <div class="form-group">
                  <label>{{ __('locale.Icon') }} ({{ __('locale.JPG/PNG') }})</label>
                  ...
                  <small class="text-muted">{{ __('locale.JPG/PNG up to 5MB') }}</small>
              ...
              <a href="{{ route('payment-gateway.index') }}" class="btn btn-outline-secondary mr-2">{{ __('locale.Back') }}</a>
              <button type="submit" class="btn btn-primary">{{ $gateway ? __('locale.Update') : __('locale.Save') }}</button>
            </div>
          </form>
        </div>
      </div>
