                  ...
                  <tr>
                    ...
                    <td>
                      <div class="d-flex align-items-center col-actions justify-content-end" style="min-width:170px;">
                        <a href="{{ route('discounts.edit', $discount) }}" class="mr-1" data-toggle="tooltip" data-placement="top" title="{{ __('Edit') }}">
                          <i data-feather="edit-2"></i>
                        </a>
                        <form class="m-0 mr-1" action="{{ route('discounts.status', $discount) }}" method="POST">
                          @csrf
                          <button type="submit" class="btn btn-icon btn-flat-{{ $discount->status === \App\Models\Discount::ACTIVE ? 'success' : 'secondary' }}" data-toggle="tooltip" data-placement="top" title="{{ __('Toggle status') }}">
                            <i data-feather="{{ $discount->status === \App\Models\Discount::ACTIVE ? 'toggle-right' : 'toggle-left' }}"></i>
                          </button>
                        </form>
