...
@section('title', isset($rate) ? __('locale.Edit Exchange Rate') : __('locale.New Exchange Rate'))
  ...
  <div class="row">
    <div class="col-12">
      <div class="card">
          ...
          <h4 class="card-title">{{ isset($rate) ? __('locale.Edit Exchange Rate') : __('locale.New Exchange Rate') }}</h4>
        </div>
        <div class="card-body">
          @if($errors->any())
          ...
          <form id="exchange-rate-form" method="POST" action="{{ isset($rate) ? route('exchange-rates.update', $rate->id) : route('exchange-rates.store') }}">
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                  <label for="rate_date">{{ __('locale.Date Recorded') }}</label>
              ...
              <div class="col-md-3">
                <div class="form-group">
                  <label for="currency_from">{{ __('locale.Currency From') }}</label>
              ...
              <div class="col-md-3">
                <div class="form-group">
                  <label for="currency_to">{{ __('locale.Currency To') }}</label>
            ...
            <div class="mt-4 d-flex justify-content-end">
              <a href="{{ route('exchange-rates.index') }}" class="btn btn-outline-secondary mr-2">{{ __('locale.Back') }}</a>
              <button id="exchange-rate-submit" type="submit" class="btn btn-primary">
                <span id="exchange-rate-submit-text">{{ isset($rate) ? __('locale.Update') : __('locale.Save') }}</span>
    ...
    (function(){
      ...
      form.addEventListener('submit', function(e){
        ...
        const proceed = () => {
          ...
          submitText.textContent = isUpdate ? '{{ __('locale.Updating...') }}' : '{{ __('locale.Saving...') }}';
          form.submit();
        };

        if (isUpdate) {
          Swal.fire({
            title: '{{ __('locale.Confirm update') }}',
            text: '{{ __('locale.Are you sure you want to update this rate?') }}',
            icon: 'warning',
            showCancelButton: true,
            confirmButtonText: '{{ __('locale.Yes, update') }}',
