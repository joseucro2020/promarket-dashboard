...
<head>
  ...
  <style>
    ...
    .items-title, .delivery-title { margin: 18px 0 10px; font-size: 18px; font-weight: 600; }
    .intro-text, .contact-text { margin: 4px 0; font-size: 14px; line-height: 1.5; }
...
<body>
  ...
  <div class="container">
      ...
      <tbody>
        @foreach($compra->details as $item)
          <tr class="borde-blanco">
            ...
            <td class="text-center">{{ Money::getByCurrency(CalcPrice::getByCurrency($item->price, $item->coin, optional($compra->exchange)->change, $compra->currency), $compra->currency) }}</td>
            <td class="text-right">{{ Money::getByCurrency(CalcPrice::getByCurrency($item->price, $item->coin, optional($compra->exchange)->change, $compra->currency) * $item->quantity, $compra->currency) }}</td>
          </tr>
        @endforeach
      </tbody>
      <tfoot>
        ...
        <tr class="text-justify">
          ...
          <td class="money" colspan="">
            <p>{{ Money::getByCurrency(CalcPrice::getByCurrency($compra->tip_money, data_get($compra, 'details.0.coin'), optional($compra->exchange)->change, $compra->currency), $compra->currency) }}</p>
          </td>
        </tr>
        <tr class="text-justify">
          <td colspan="3">
          ...
          <td class="money" colspan="">
            <p>{{ Money::getByCurrency(CalcPrice::getByCurrency($compra->shipping_fee, data_get($compra, 'details.0.coin'), optional($compra->exchange)->change, $compra->currency), $compra->currency) }}</p>
          </td>
        </tr>
        <tr class="text-justify">
          <td colspan="3">
          ...
          <td class="money" colspan="">
            <p>{{ $compra->text_payment_type }} {{ $compra->use_balance == '1' ? ' + Saldo' : '' }}</p>
          </td>
        </tr>
        <tr class="text-justify">
          <td colspan="3">
    ...
    <div class="row">
      ...
      @endif
      <div class="col-12">
        <p class="contact-text">Dirección de entrega: {{ optional($compra->delivery)->address }}</p>
      </div>
      <div class="col-12">
        <p class="contact-text">
          Fecha: {{ optional($compra->delivery)->date_formated }}  -
          Turno: {{ optional($compra->delivery)->turn_formated }}
        </p>
      </div>
    </div>
    <div class="row">
      ...
      @if($compra->payment_type == 1 || $compra->payment_type == 2)
        <div class="col-12">
          <p class="contact-text">
            Banco: {{ optional(optional(optional($compra->transfer)->bankAccount)->bank)->name }} -
        ...
        <div class="col-12">
          <p class="contact-text">
            Nro. Referencia: {{ optional($compra->transfer)->number }}
          </p>
        </div>
      @endif
      @if($compra->payment_type == 3)
        <div class="col-12">
          <p class="contact-text">
            Nombre: {{ optional($compra->transfer)->name }}
          </p>
        </div>
        <div class="col-12">
          <p class="contact-text">
      ...
      @if($compra->payment_type == 6)
        <div class="col-12">
          <p class="contact-text">
            Código de transacción: {{ optional($compra->transfer)->number }}
          </p>
        </div>
      @endif
    </div>
